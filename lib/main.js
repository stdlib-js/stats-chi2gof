...
function chi2gof( x, y ) {
	...
	if ( isndarrayLike( x ) && x.ndims === 1 && x.strides.length === 1 ) { // is ndarray-like vector?
		...
		throw new TypeError( format( '1Gz3T', x ) );
	}
	N = x.length;
	...
	for ( i = 0; i < N; i++ ) {
		...
		if ( !isNonNegativeInteger( v ) ) {
			throw new TypeError( format( '1Gz9L', i, v ) );
		}
		obs[ i ] = v;
		n += v;
	}
	if ( n === 0 ) {
		throw new Error( format('1Gz1D') );
	}
	...
	if ( isString( y ) ) {
		...
		for ( i = 0; i < nargs; i++ ) {
			...
			if ( !isNumber( v ) || isnan( v ) ) {
				throw new TypeError( format( '1Gz9M', i+2, v ) );
			}
			args.push( v );
		}
		...
		if ( isndarrayLike( y ) && y.ndims === 1 && y.strides.length === 1 ) { // is ndarray-like vector?
			...
			throw new TypeError( format( '1Gz9N', y ) );
		}
		if ( y.length !== N ) {
			throw new RangeError( format('1Gz1E') );
		}
		...
		for ( i = 0; i < N; i++ ) {
			v = d[ o+(s*i) ];
			if ( !isNumber( v ) ) {
				throw new TypeError( format( '1Gz9O', i, v ) );
			}
			if ( v < 0.0 ) {
				throw new TypeError( format( '1Gz9P', i, v ) );
			} else if ( v > 1.0 ) {
				sum += PINF;
			} else {
				sum += v;
